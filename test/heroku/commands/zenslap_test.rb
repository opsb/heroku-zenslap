...
class ZenslapTest < Test::Unit::TestCase
  ...
  context "zenslap add command" do
    ...
    setup do
      ...
      RestClient.stubs(:post).with "http://zenslap.heroku.com/heroku/resources", 
                                   :github_url => "git@github.com:opsb/zenslap-heroku"
      @command.add      
    end
    
    should "provision continuous integration through zenslap API" do
      assert_received RestClient, :post do |expect| 
        expect.with "http://zenslap.heroku.com/heroku/resources", 
                    :github_url => "git@github.com:opsb/zenslap-heroku" 
      end
    end
